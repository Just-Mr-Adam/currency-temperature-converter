namespace ConverterApp.Converters
{
    public class TemperatureConverter : BaseConverter
    {
        ...
        {
            return new string[] { "Цельсия °C", "Фаренгейт °F", "Кельвин K" };
        }
        ...
        {
            if (fromUnit == "Цельсия °C") return value;
            if (fromUnit == "Фаренгейт °F") return (value - 32) * 5 / 9;
            if (fromUnit == "Кельвин K") return value - 273.15;
            throw new ArgumentException("Неизвестная единица: " + fromUnit);
        }

        private double FromCelsius(double celsius, string toUnit)
        {
            if (toUnit == "Цельсия °C") return celsius;
